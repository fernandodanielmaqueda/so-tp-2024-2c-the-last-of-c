...
int list_add_unless_any(t_list *list, void *data, bool (*condition)(void *, void*), void *comparation) {
    ...
    if(new_element == NULL) {
        log_warning_r(&MODULE_LOGGER, "malloc: No se pudieron reservar %zu bytes para un nuevo elemento de la lista", sizeof(t_link_element));
        return -1;
    }
...
int shared_list_init(t_Shared_List *shared_list) {
	...
	if(shared_list->list == NULL) {
		log_warning_r(&MODULE_LOGGER, "shared_list_init: No se pudo crear la lista");
		retval = -1;
		goto cleanup_mutex;
	}
